...
client.on('message', (message) => {
  ...
  if (command === 'play') {
    ...
    client.commands.get('play').execute(args, client, dispatchers, globalState, message, voiceChannel);
    ...
    client.commands.get('next').execute(args, client, dispatchers, globalState, message);
  } else if (command === 'previous') {
    client.commands.get('previous').execute(args, client, dispatchers, globalState, message);
    ...
    const voiceChannel = message.member.voice.channel;
    client.commands.get('loop').execute(client, dispatchers, globalState, message, voiceChannel)
  }
});
