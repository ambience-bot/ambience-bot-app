...
client.on('message', (message) => {
  ...
  if(command === 'play' || command === 'favorites') {
    if(!globalState.hasOwnProperty(guild.id)) {
      globalState[guild.id] = {};
      client[`${guild.id}SongIds`] = new Discord.Collection();
      client[`${guild.id}Favorites`] = new Discord.Collection();
    }
    globalState[guild.id].id = 0;
  } 

  client.commands.get(command).execute({args, client, dispatchers, globalState: globalState[guild.id], message, voiceChannel})
});
